# Harém Brasil — API REST + tempo real: implementação em Go
...
1. **Prefixo:** `https://api.harembrasil.com.br/api/v1` (ajustar host por ambiente).  
2. **JSON:** `Content-Type: application/json; charset=utf-8` salvo `multipart` ou rotas binárias.  
3. **UTF-8:** validar tamanho em **bytes** e em **runes** em texto livre crítico (`utf8.RuneCountInString` + limite de bytes no middleware).  
...
6. **Whitelist** em todos os `POST`/`PATCH` (structs com tags + validação explícita ou decoder custom).  
7. **BOLA:** validar posse ou papel em `{user_id}`, `{post_id}`, etc.  
8. **BFLA:** `/admin/*` com middleware e política isolada.  
9. **Logging:** `slog` ou equivalente estruturado; eventos de segurança sem PII (ver secção 11).  
10. **Rate limit:** por IP e por utilizador; cabeçalhos `RateLimit-*` / `Retry-After` conforme política.
...
| `DELETE` | Remoção lógica (`deleted_at`) salvo purge admin. |
...
- `cursor` — string opaca (ex.: base64url de `(created_at, id)`).
...
- `snake_case` nas query keys; rejeitar filtros não documentados (evitar abuso / injection lógica).
...
- **Access JWT** curto (10–15 min); **refresh** opaco com hash em PostgreSQL (ou Redis) e **rotação**.  
- Claims: `sub`, `iss`, `aud`, `exp`, `iat`, `jti`, `role` ou `roles`.  
- Validar assinatura, `exp`, `iss`, `aud`; opcional denylist por `jti`.  
- **OAuth/OIDC:** Authorization Code + PKCE; handlers Go trocam `code` e emitem par Harém.

**Go:** armazenar refresh com **Argon2id** ou **bcrypt** do segredo/opaco; nunca logar tokens.
...
```json
{
  "type": "https://api.harembrasil.com.br/problems/validation-error",
  ...
  "request_id": "01JR…",
  "errors": [
    { "field": "body", "code": "too_long", "message": "Body exceeds maximum length" }
  ]
}
```
...
Permissões exemplo: `post.create`, `post.create_paid`, `forum.moderate`, `chat.public_write`, `billing.manage_self`, `creator.verification.submit`.
...
**Logout (semântica):** `POST /auth/logout` revoga o refresh token apresentado (ou a sessão corrente); `POST /auth/logout-all` revoga todos os refresh tokens do utilizador. Ambos são **`POST`**; exigem autenticação (`Authorization: Bearer …` e/ou cookie de sessão, conforme implementação) e corpo vazio ou JSON mínimo conforme OpenAPI — nunca `GET`.

**Registo — whitelist:** `email`, `password`, `screen_name`, `accept_terms_version`.  
**Resposta `201`:** `user` mínimo + `tokens` (`access_token`, `access_expires_in`, `refresh_token`, `refresh_expires_in`).

### 6.3 `/me`

- `GET` / `PATCH`. **Whitelist PATCH:** `screen_name`, `bio`, `locale`, `notify_preferences` (subchaves permitidas).

### 6.4 Utilizadores públicos

- `GET /users/{user_id}`, `/users/{user_id}/posts`, `GET /users/search` (`q`, `limit`, `cursor`).

### 6.5 Posts e feed

- CRUD `/posts`, `/posts/{post_id}`, likes, comments, `GET /feed/home`.  
- **POST /posts whitelist:** `body`, `visibility`, `media[]` com `upload_id`, `kind`, `alt_text`.

### 6.6 Fórum

- Categorias, tópicos, respostas; no código usar `ForumReply` / `forum_posts` para não colidir com `posts` do feed.

### 6.7 Chat (REST)

- **Histórico de mensagens (caminho canônico):** `GET /chat/rooms/{room_id}/messages` com query `cursor`, `limit`, `before`. Salas e membros seguem o mesmo prefixo (`/chat/rooms`, `/chat/rooms/{room_id}`, `…/members`, etc., no OpenAPI). Envio em tempo real via WebSocket (secção 8).

### 6.8 Assinaturas

- `GET /billing/plans`, `POST /billing/checkout` (**Idempotency-Key**), `GET /billing/subscription`, cancel/resume.  
- Estado final coerente após webhook + BD.

### 6.9 Webhooks

- `POST /webhooks/stripe|pagseguro|mercadopago` — corpo cru para HMAC; `200` rápido + fila; idempotência por `event_id`; logs sem payload completo.

### 6.10 Criador

- Verificação, catálogo, pedidos, ganhos; whitelist de item: `title`, `description`, `price_cents`, `currency`, `visibility`, `media`.
...
- `POST /media/upload-sessions`; opcional `POST .../{upload_id}/complete` para validação server-side.

### 6.13 Admin — prefixo canônico `/admin/*`

- **Caminho:** todas as rotas administrativas ficam sob **`/admin/*`** relativamente a `/api/v1` (ex.: `/api/v1/admin/users/…`). Isolado com middleware próprio, só papel `admin`, auditoria; nunca `password_hash`, tokens ou documentos em base64 na resposta.
...
- Eventos servidor → cliente: `chat.message`, `chat.typing`, `notification`, `subscription.updated`.  
- Ao subscrever `room_id`, verificar membro em PostgreSQL (cache curto Redis opcional).  
- Rate limit por utilizador e por sala.
...
| `internal/middleware` | Request ID, auth, rate limit, recover, logging, `MaxBytesReader`. |
| `internal/realtime` | Hub WebSocket, tickets, quotas. |
